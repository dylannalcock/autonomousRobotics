...
class ParticleInitializer:
    ...
    def reset_click_pose(self, msg, particles, weights):
        ...
        particles[:,0] = np.random.normal(msg.position.x, np.sqrt(self.x_std), size=(n_particles))
        particles[:,1] = np.random.normal(msg.position.y, np.sqrt(self.y_std), size=(n_particles))
        particles[:,2] = np.random.normal(utils.quaternion_to_angle(msg.orientation), np.sqrt(self.theta_std), size=(n_particles))
        # END QUESTION 3.1

...
class SingleBeamSensorModel:
    ...
    def precompute_sensor_model(self, max_r):
        ...
        hit_mask = np.logical_and(obs_r <= max_r, 0 <= obs_r)
        if self.hit_std == 0:
            p_hit[hit_mask] = obs_r[hit_mask]
        else:
            p_hit[hit_mask] = (1.0 / (np.sqrt(2.0 * np.pi * (self.hit_std ** 2.0)))) * np.exp(-0.5 * ((obs_r - sim_r) / self.hit_std) ** 2.0)[hit_mask]
        
        p_short = np.zeros((table_width, table_width))
        short_mask = obs_r < sim_r
        p_short[short_mask] = 2 * ((sim_r - obs_r) / sim_r)[short_mask]

        p_max   = np.zeros((table_width, table_width))
        max_mask     = obs_r == max_r
        p_max[max_mask] = 1

        p_rand = np.zeros((table_width, table_width))
        rand_mask = np.logical_and(obs_r < max_r, 0 <= obs_r)
        if (max_r != 0):
            p_rand[rand_mask] = 1/max_r

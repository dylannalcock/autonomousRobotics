...
def compute_position_in_frame(p, frame):
    ...
    difference = p[0:2] - frame[0:2]
    cos = np.cos(frame[2])
    sin = np.sin(frame[2])
    rotate     = np.array([[cos, sin], [-sin, cos]])
    return np.dot(rotate, difference)
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            # BEGIN QUESTION 1.1
            "*** REPLACE THIS LINE ***"
            distances = np.linalg.norm(path_xytv[:,:2] - pose[:2], axis=1)

            closest = np.argmin(distances, 0)
            """
            distances = distances[closest:]

            mask = distances >= distance_lookahead

            too_close = len(distances) - len(distances[mask])

            distances = distances[mask]

            result = np.argmin(distances, 0)
            """
            for i in range(closest, len(distances)):
                if distances[i] >= distance_lookahead:
                    return i
            #look = distances >= distance_lookahead
            #res = np.nonzero(look)
            #if len(res) > 0:
            #    return res[0]
            #else:
            ...
            return len(distances) - 1
            # END QUESTION 1.1
            #return result + closest + too_close
    ...
    def get_error(self, pose, reference_xytv):
        ...
        #return np.array([reference_xytv[0] - pose[0], reference_xytv[1] - pose[1]])
        raise NotImplementedError
    ...
    def get_control(self, pose, reference_xytv, error):
        ...
        #return np.array([reference_xytv[3], -((KP*error[1]) + 
        #                                    (KD * (reference_xytv[3] * np.sin(pose[2] - reference_xytv[2]))))])

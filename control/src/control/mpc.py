...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        #    controls[:,i,1] = np.linspace(self.min_delta, self.max_delta, num=self.K)
        #controls[:,:,1] = np.linspace(self.min_delta, self.max_delta, num=self.K, axis=0)
        ax = np.empty((self.K, 2))
        ax[:,1] = np.linspace(self.min_delta, self.max_delta, num=self.K)
        controls = np.repeat(ax[:, None, :], self.T, axis=1)
        #    for j in range(len(controls[i])):
        #        controls[i,j,1] = np.linspace(self.min_delta, self.max_delta, num=2)
        # END QUESTION 4.1
        return controls
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        for i in range(len(controls[0])):
            new_rollout = self.motion_model.compute_changes(rollouts[:, i, :], 
                                                            controls[:, i, :], dt)
            rollouts[:, i+1, :] = rollouts[:, i, :] + new_rollout

        #raise NotImplementedError
        print("HEHEHE:", self.max_delta)
        # END QUESTION 4.2
        return rollouts
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        #raise NotImplementedError
        distance = np.sqrt(
                        np.square(reference_xyt[0] - rollouts[:,-1,0])
                        + np.square(reference_xyt[1] - rollouts[:,-1,1]))
        distance *= self.error_w
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        "*** REPLACE THIS LINE ***"
        #raise NotImplementedError
        collisions = np.empty(shape=(self.K, self.T + 1))

        #collisions = np.apply_along_axis(self.check_collisions_in_map, 1, rollouts)
        #for i in range(len(rollouts)):
            #collisions[i] = self.check_collisions_in_map(rollouts[i])
        #print("shape: " + str(np.reshape(rollouts, (self.K * (self.T + 1), 3)).shape))
        output = self.check_collisions_in_map(np.reshape(rollouts, (self.K * (self.T + 1), 3)))
        collisions = np.reshape(output, (self.K, self.T + 1))

        costs = self.collision_w * np.sum(collisions, axis=1)
        for i in range(len(costs)):
            if i > 0:
                costs[i] += 0.1 * costs[i-1]
            elif i < len(costs) - 1:
                costs[i] += 0.1 * costs[i+1]
        return costs
        # END QUESTION 4.3
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        # BEGIN QUESTION 4.4
        "*** REPLACE THIS LINE ***"
        costs = self.compute_rollout_cost(rollouts, reference_xytv[0:3])
        # END QUESTION 4.4
        ...
        #raise NotImplementedError
        idx = np.argmin(costs)
        #return rollouts[idx, 0, :]
        control = np.copy(self.sampled_controls[idx,0,:])
        control[0] = reference_xytv[3] - (1.875 * np.abs(control[1]))
        return control
        #return self.sampled_controls[idx,0,:]
        # END QUESTION 4.4

    ...
    while len(queue) > 0:
        ...
        for neighbor, w in rm.graph[entry.node].items():
            ...
            if not expanded[neighbor]:
                queue.push(QueueEntry(h + g, next(c), neighbor, entry.node, g))
            # END QUESTION 2.1
...
def shortcut(rm, vpath, num_trials=100):
    ...
    for _ in range(num_trials):
        ...
        "*** REPLACE THIS LINE ***"
        #raise NotImplementedError
        new_len = rm.compute_path_length(vpath[:i+1]) + rm.heuristic(i, j) #+ rm.compute_path_length(vpath[j:]) 
        #new_len = rm.compute_path_length(vpath[:i+1] + vpath[j:])
        if rm.check_edge_validity(vpath[i],vpath[j]) and rm.compute_path_length(vpath[:j + 1]) > new_len:

...
class PlanarProblem(object):
    ...
    def check_state_validity(self, states):
        ...
        # BEGIN QUESTION 1.2
        "*** REPLACE THIS LINE ***"
        x_mask = (x >= self.extents[0,0]) & (x < self.extents[0,1])
        y_mask = (y >= self.extents[1,0]) & (y < self.extents[1,1])
        valid &= x_mask 
        valid &= y_mask
        # END QUESTION 1.2
        ...
        # BEGIN QUESTION 1.2
        "*** REPLACE THIS LINE ***"
        x = states[:,0].astype(int)
        y = states[:,1].astype(int)
        #valid = valid & self.permissible_region[y[y_mask],x[x_mask]]
        #np.where(valid, self.permissible_region[y[valid],x[valid]], False)
        
        no_coll = self.permissible_region[y[valid],x[valid]]
        #raise ValueError(no_coll.shape)
        valid[valid] = no_coll
        #np.where(valid, valid & self.permissible_region[states[:,1], states[:,0]], False)
        
        #valid = np.logical_and(valid, self.permissible_region[y,x])
        #for i in range(len(states)):
        #    x = states[i,0].astype(int)
        #    y = states[i,1].astype(int)
        #    if (valid[i]) :
        #        valid[i] &= self.permissible_region[y, x]

...
class Roadmap(object):
    ...
    def check_weighted_edges_validity(self, weighted_edges):
        ...
        # END QUESTION 1.3
        ixs = []  #self.check_edge_validity(weighted_edges[:,0].astype(int), weighted_edges[:,1].astype(int))
        for i in range(len(weighted_edges)):
            if self.check_edge_validity(weighted_edges[i,0].astype(int), weighted_edges[i,1].astype(int)):
                ixs.append(i)
        return weighted_edges[ixs]
